...
import { Strategy, RiskLevel, VisibilityType, TECHNICAL_STRATEGIES, TechnicalStrategy, AgentConfig, BacktestResult } from "@/types/strategy";
...
const CreateStrategyPage = () => {
    ...
    const [generatedCode, setGeneratedCode] = useState({ python: "", pinescript: "" });
    ...
    const [showBacktestResultsModal, setShowBacktestResultsModal] = useState(false);
    const [viewingBacktestStrategyId, setViewingBacktestStrategyId] = useState<string | null>(null);

    const handleInputChange = (field: string, value: any) => {
        setFormData({ ...formData, [field]: value });
    };

    const handleAgentChange = (agent: 'supervisor' | 'executor' | 'technical' | 'sentiment' | 'webSearch', field: 'prompt' | 'weightage', value: string | number) => {
        setFormData({
            ...formData,
            [agent]: {
                ...formData[agent],
...
plot(sma50, color=color.red)`;
    ...
    const weightageSum = (formData.technical.weightage || 0) +
                         (formData.sentiment.weightage || 0) +
                         (formData.webSearch.weightage || 0);
    const isWeightageValid = weightageSum === 100;
    const isStep2Valid = formData.depositAmount && parseFloat(formData.depositAmount) > 0 && isWeightageValid;
                        ...
                        <Card title="AI Agents Configuration">
                            <div className="space-y-8">
                                ...
                                <div className="p-6 bg-theme-on-surface-1 rounded-xl border border-theme-stroke">
                                    ...
                                    {formData.selectedTechnicalStrategies.filter((t) => t.isCustom).length > 0 && (
                                        <div className="mb-6">
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div>
                                        ...
                                        <textarea
                                            ...
                                            onChange={(e) => setGeneratedCode({...generatedCode, python: e.target.value})}
                                    ...
                                    <div>
                                        ...
                                        <textarea
                                            ...
                                            onChange={(e) => setGeneratedCode({...generatedCode, pinescript: e.target.value})}
                                            className="w-full h-64 p-4 bg-theme-on-surface-2 border border-theme-stroke rounded-xl text-caption-1 font-mono text-theme-primary resize-none"
                                        />
                                    </div>
